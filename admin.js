/* ==========================================================================
   👑 KINGDOM HOTEL - ADMIN DASHBOARD JAVASCRIPT (FULLY FIXED)
   ========================================================================== */
...
// 🛠️ ፊክስ: 4 ካርዳት (Total, Pending, Cancelled, Revenue) ዝሰርሕ ሎጂክ
function renderTable(bookingsToShow, isFiltering = false) {
    ...
    if (bookingsToShow.length === 0) {
        tbody.innerHTML = `<tr><td colspan='7' class='no-data-row'><i class='fas fa-exclamation-triangle'></i> ኣብዚ ዝመረጽካዮ ዕለት ወይ ሽም ዝተረኽበ ዳታ የለን!</td></tr>`;
        return;
    }
    ...
    bookingsToShow.forEach(book => {
        ...
        const row = `
            <tr>
                ...
                <td>
                    <button class="btn table-confirm-btn" onclick="updateStatus(${book.row}, 'Confirmed', this)" ${isDisabled}>Confirm</button>
                    <button class="btn table-cancel-btn" onclick="updateStatus(${book.row}, 'Cancelled', this)" ${isDisabled}>Cancel</button>
                </td>
            </tr>
        `;
        tbody.innerHTML += row;
...
// 🛠️ ፊክስ: CORS Error ዝተኣለየሉ ዘመናዊ Update Status
function updateStatus(row, newStatus, buttonElement) {
    ...
    confirmMessage.innerText = `ነዚ ምዝገባ ናብ "${newStatus}" ክትቅይሮን ንዓሚል ናይ SMS/ኢመይል መልእኽቲ ክትሰደሉን ርግጸኛ ዲኻ?`;
    confirmOverlay.style.display = "flex"; 

    noBtn.onclick = () => confirmOverlay.style.display = "none";
    ...
    yesBtn.onclick = async function() {
        ...
        try {
            ...
            setTimeout(() => fetchAllBookings(), 1500);
        } catch (e) {
...
function filterBookings() {
    ...
    const filtered = window.allBookingsData.filter(book => {
        return (book.name || "").toLowerCase().includes(filter) || 
               (book.room || "").toLowerCase().includes(filter) || 
               String(book.phone || "").toLowerCase().includes(filter);
    });
...
function showCustomAlert(message) {
    ...
    if (alertBox && alertMsg) { alertMsg.innerText = message; alertBox.classList.remove("hidden"); }
}
function closeCustomAlert() { const alertBox = document.getElementById("customAlert"); if (alertBox) alertBox.classList.add("hidden"); }
